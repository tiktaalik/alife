...
#==============================================================================
    ...
    def output_path(self, out_variety):
        ...
        output_file = "%s_rec-%d_par-%d_gen-%i_%s.csv" % (mode, self.num_records/1000, self.num_parents, self.gen_len, self.dist)
...
#==============================================================================
    ...
    def citing(self):
        ...
        if dist == 'flat' or 'ave'
            ...
            x = np.random.poisson(num_parents)
...
class Aging(Patents):
    ...
    def update_weights(self):
        ...
        self.weights = [1 * self.aging_coefficients[i] for i in range(self.now_forming)]
...
class Keywords(object):
    ...
    def __init__(self, parentage_file, traits_file=None, num_traits=5, avg=True, min_traits=0, num_keywords=100, gen_len=100):
...
#        if self.traits_file != None:
#            self.open_t_file(self.traits_file)
...
#==============================================================================
    def write_phenomes(self):   
        with open('phenomes.csv', 'w') as file:
...
""" % layer_select
        ...
        # guide rank
        file.write('/*guide*/ { rank = same; ')
        for i in range(self.num_records//self.gen_len - 1):
            s = ('gen_',str(i),' -> ')
            s = ''.join(s)      
            file.write(s)
        file.write(str(self.num_records//self.gen_len-1))
        file.write(';}\n')
        ...
        for i in range(self.num_records//self.gen_len):
            ...
            for j in range(self.gen_len):
                ...
                if 'phylo' in focus:
                     ...
                     if (selected_layer in self.ancestors[rec]):             
                        s = (str(rec),'; ')
                        s = ''.join(s)           
                        file.write(s)
                # traits
                else:
                    # children in inheritance interactions                    
                    print(self.inheritance_interactions)      
                    ii_set = [set(i) for i in self.inheritance_interactions]
                    # parents in inheritance interactions 
                    for i in range(self.num_records):
                        if len(self.inheritance_interactions[i]) != 0:
                            ii_set.add(i)
...
class Testing(object):
    ...
    def lets_cite(self):
        self.some_patents = Preferential(num_records=100, num_parents=10, avg=False,
                 min_parents=0, gen_len=10)
    ...
    def lets_keyword(self):
        self.key_up = Keywords("parentage.csv", num_traits=5, avg=False,
                               min_traits=0, num_keywords=500, gen_len=10)
...
#        self.key_up.write_inheritance_count()
        ...
        for layer in layers:
            self.key_up.genealogy_dot(layer, "keyword")
...
#
        self.key_up.get_phylogenies() 
        some_ancestors = [0, 25, 100]
        for ancestor in some_ancestors:
            self.key_up.genealogy_dot(ancestor, "phylo")

#        print(self.key_up.inheritance_average_random())
#        print(self.key_up.inheritance_average_related())
