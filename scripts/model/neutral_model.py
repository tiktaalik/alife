...
#==============================================================================
    ...
    def citing(self):
        ...
        if self.dist == 'poisson':
            x = 0
            while x == 0:
                x = numpy.random.poisson(self.num_parents) #x = randint(self.min_parents, self.max_parents)
        """    
        if self.dist == 'flat' or 'ave':
...
class Aging(Patents):
    ...
    def define_prior_to_forming(self):
        """Calls the procedures requisite for pate nt formation"""
...
#==============================================================================
    ...
    def genealogy_dot(self, selected_layer, focus, count):
        # In command line: dot -Kfdp -n -Textension -o out_name.extension in_name.dot
        # e.g., dot -Kfdp -n -Tps -o sample.ps  dot_for_graphviz.dot (prints paths which can be opened in Illustrator)
        
        interest = count
        descendents = self.descendents[interest]
...
""" % layer_select
        ...
        for patent, phenome in enumerate(self.phenomes):
            if interest in self.ancestors[patent] or patent == interest:
...
class PriceEquation(NetworkAnalysis):
    def __init__(self):
        super(PriceEquation, self).__ini__()
...
class Testing(object):
    ...
    def lets_cite(self):
        self.some_patents = PrefAging(num_records=self.num_records, 
...
median_writer = csv.writer(median_file, delimiter=' ')
max_writer = csv.writer(max_file, delimiter=' ')
median_writer.writerow(median_reader)
median_file.close()
max_file.close()
