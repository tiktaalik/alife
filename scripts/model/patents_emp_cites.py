...
#==============================================================================
    ...
    def generation(self):
        ...
        while (self.gen_num + 1) * self.gen_len > self.now_forming and self.num_records - self.now_forming > 1:
                self.citing()
    ...
    def citing(self):
        ...
        if self.dist == 'poisson':
            x = np.random.poisson(self.num_parents)
        else:
            x = np.random.randint(self.min_parents, self.max_parents+1) # randint is high exclusive
    ...
    def update_weights(self):
        ...
        # if all weights are 0, set all to 1
        if self.summed_weights == 0:
            self.weights = [1]*len(self.weights)
            self.summed_weights = len(self.weights)
...
class Empirical(Patents):

    def define_prior_to_forming(self):
        """Calls the procedures required for patent formation"""
        super(Empirical, self).define_prior_to_forming()
        self.define_ages()
        self.load_surfaces()
        self.emp_gen_len()

    def load_surfaces(self):
        """Empirical probabilities of being cited given age and prior hit
         count.
        The hits surface is a matrix H whose entry in the a^th row and 
        c^th column is H[a,c] = (number of cites such that age of parent at 
        time of cite=a, prior cites to parent at time of cite=c)."""

        self.hits_surface = np.load("hits_surface.npy")
        #self.existence_surface = np.load("existence_surface.npy")
        hs_sum = self.hits_surface.sum()

        # divide each value of the matrix by the sum to yield probabilities
        self.hits_surface = self.hits_surface/float(hs_sum)
    
    def emp_gen_len(self):
        self.gen_len = self.num_records/len(self.hits_surface)
    
    def form_patents(self):
        """Initiates the formation of patents"""
        self.now_forming = self.gen_len
        self.update_weights()
        super(Empirical, self).form_patents()
    ...
    def define_ages(self):
        """Get the ages (in weeks) for the first generation"""
        self.ages = np.array([0]*self.gen_len)
    
    def update_weights(self):
        """Updates the weights and probs according the patent's age and
         prior hit count.""" 
        self.update_ages()

        self.weights = [0]*self.now_forming
        for i in range(self.now_forming):
            age = self.ages[i] - 1
            cites = self.citation_count[i] - 1 
            # grab the weight
            self.weights[i] = self.hits_surface[age][cites]

        super(Empirical, self).update_weights()

    def update_ages(self):
        self.ages += 1
        new_ages = np.array([1]*self.gen_len)
